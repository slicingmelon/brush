...
For Cycle 5 — **research complete 2026-04-25**:
- [x] Research doc at [`docs/research/grep-sed-awk-options.md`](../research/grep-sed-awk-options.md) with concrete recommendation per utility.
- [x] No code changes (intentionally).
- [x] Per-utility decision: grep → defer indefinitely (no embeddable Rust grep); sed → track `uutils/sed` (clean integration when v0.2.0+ ships); awk → defer indefinitely (no usable Rust port).
...
| 2026-04-25 | Cycle 5 (research complete) | Research doc landed at `docs/research/grep-sed-awk-options.md`. Findings per utility: **grep** — no `uutils/grep` exists; ripgrep is mature (62k★) but binary-only (no embeddable lib API), and library crates ship the engine not a CLI dispatcher. Three integration paths considered (binary-embedding, CLI-on-top-of-libs, flag-translation-to-rg); none small enough for this cycle. **Defer indefinitely**, document as PATH fallthrough. **sed** — `uutils/sed` v0.1.1 exists and is active, has standard `uumain(args)` API, pins `uucore = "0.8.0"` matching ours (zero skew). Cleanest integration of any non-coreutils tool surveyed; could plug into existing `register!` macro with one line + dep entry. But 0.1.1 is pre-feature-complete. **Track upstream**, re-evaluate at v0.2.0+. **awk** — no `uutils/awk` exists; `frawk` is binary-only and self-described as "Awk-like" not POSIX. **Defer indefinitely**, document as PATH fallthrough. Net: only sed has a viable Rust path; grep and awk should remain PATH fallthrough until the Rust ecosystem matures. | `docs/research/grep-sed-awk-options.md`; gh-fetched repo metadata for `uutils/sed`, `BurntSushi/ripgrep`, `ezrosent/frawk`, `chmln/sd`, `ms-jpq/sad`, `orgs/uutils/repos`. |
