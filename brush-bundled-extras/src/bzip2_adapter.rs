//! bzip2 / bunzip2 / bzcat adapter — via the [`bzip2`] crate
//! (currently uses `libbz2-rs-sys`, a pure-Rust port).
//!
//! Per `docs/planning/bundled-extras-coverage-expansion.md` Cycle 2.
//! Same dispatch shape as `gzip_adapter`: all three names share one
//! entry point branching on `argv[0]`.
...
pub(crate) fn bunzip2_main(args: Vec<OsString>) -> i32 {
    run(args, Mode::Decompress)
}

pub(crate) fn bzcat_main(args: Vec<OsString>) -> i32 {
...
fn run(args: Vec<OsString>, default: Mode) -> i32 {
    ...
    while i < argv.len() {
        ...
        match arg.as_str() {
            ...
            "--version" => {
                println!("bzip2 (brush-bundled-extras) 0.1.5");
                return 0;
            }
            s if s.starts_with('-')
                && s.len() == 2
            ...
            s if s.starts_with('-') && s != "-" => {
                eprintln!("bzip2: unknown option: {s}");
                return 1;
            }
        ...
        Mode::Compress => {
            ...
            if let Some(out) = out_path {
                ...
                let f = OpenOptions::new()
                    ...
                    .truncate(true)
                    .open(&out)?;
                let mut enc = BzEncoder::new(BufWriter::new(f), level);
                ...
                let mut enc = BzEncoder::new(BufWriter::new(stdout.lock()), level);
                io::copy(&mut input, &mut enc)?;
                enc.finish()?;
            }
        }
...
fn append_bz2(p: &Path) -> PathBuf {
    let mut s = p.as_os_str().to_owned();
    s.push(".bz2");
    PathBuf::from(s)
}

fn strip_bz2(p: &Path) -> io::Result<PathBuf> {
    let name = p
        .file_name()
        .and_then(|n| n.to_str())
    ...
    let stripped = name
        ...
        .or_else(|| name.strip_suffix(".tbz2").map(|_| "tar"))
        .or_else(|| name.strip_suffix(".tbz").map(|_| "tar"))
        .ok_or_else(|| {
            io::Error::new(
                io::ErrorKind::InvalidInput,
                "input file does not end in .bz2 / .bz / .tbz / .tbz2",
            )
        })?;
...
fn print_help() {
    println!(
        "Usage: bzip2   [OPTIONS] [FILE...]\n  \
              ...
              bzcat   [OPTIONS] [FILE...]\n\
         \n\
         Compress / decompress files using bzip2.\n\
         \n\
         Options:\n  \
           -c, --stdout       write to stdout, keep originals\n  \
           -d, --decompress   decompress mode (bzip2 -d == bunzip2)\n  \
